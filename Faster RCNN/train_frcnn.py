...
parser.add_option("--num_epochs", type="int", dest="num_epochs", help="Number of epochs.", default=3000)
parser.add_option("--config_filename", dest="config_filename", help=
				"Location to store all the metadata related to the training (to be used when testing).",
				default="config.pickle")
parser.add_option("--output_weight_path", dest="output_weight_path", help="Output path for weights.", default='./Models/model_frcnn_0852.hdf5')
parser.add_option("--input_weight_path", dest="input_weight_path", help="Input path for weights. If not specified, will try to load default weights provided by keras.")

(options, args) = parser.parse_args()
record_path=os.path.join(r'D:\PycharmProjects\Hiddencrack\tf-keras-frcnn','record.csv')
	...
	while True:
		try:
			...
			if iter_num == epoch_length:
				...
				curr_loss = loss_rpn_regr + loss_class_cls + loss_class_regr
				iter_num = 0
				start_time = time.time()
				...
				new_row = {'mean_overlapping_bboxes': round(mean_overlapping_bboxes, 3),
						   ...
						   'curr_loss': np.round(curr_loss, 3),
